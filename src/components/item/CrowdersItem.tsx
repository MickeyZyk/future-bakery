          ...
          return (
            ...
            to={ ['exiting'].includes(transitionStatus) ? {clipPath:'inset(0% 0% 100% 0%)', ease: 'Power2.easeIn', opacity: 0 } : false } >
              <div key={this.props.i} className={`${s.figure} ${'no' + (this.props.i+1)} ${this.props.i == 0 ? 'hover_element' :''}`} onMouseEnter={this.toggleHoverEnter} onMouseLeave={this.toggleHoverLeave}>
                ...
                <img className={s.bw} style={this.state.hover ? {opacity:0}:{opacity:1}} src={ 'http://futurebakery.appio.cz/grav/en/crowders/work/'+ slug(this.props.data.node.title.toLowerCase()) + '/' + this.props.data.node.big_image}/>                        
                <img className={s.bw_overlay} style={this.state.hover ? {opacity:1}:{opacity:0}} src="../images/crowders_overlay.png"/>                                 
                <img className={s.color} style={this.state.hover ? {opacity:1}:{opacity:0}} src={ 'http://futurebakery.appio.cz/grav/en/crowders/work/'+ slug(this.props.data.node.title.toLowerCase()) + '/' + this.props.data.node.big_image} />
              </div>
            </Tween>                   
          )
        }}
